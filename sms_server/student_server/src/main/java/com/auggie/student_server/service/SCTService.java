...
public class SCTService {
    ...
    public SCTInfo findByIdWithTerm(Integer sno, Integer cid, Integer tid, String term) {
        SCTInfo sctInfo=new SCTInfo();
        if (!studentCourseTeacherMapper.findBySearch(
                sno, null, 0,
                cid, null, 0,
                tid, null, 0,
                null, null, term).isEmpty()){
            sctInfo=studentCourseTeacherMapper.findBySearch(
                    sno, null, 0,
                    cid, null, 0,
                    tid, null, 0,
    ...
    public List<SCTInfo> findBySearch(Map<String, String> map) {
        Integer sno = null, cid = null, tno = null;
        ...
        if (map.containsKey("sno")) {
            try {
                sno = Integer.parseInt(map.get("sno"));
            }
            catch (Exception e) {
            }
        }
    ...
    public boolean deleteBySid(Integer sid) {
        return studentCourseTeacherMapper.deleteBySid(sid);
    }
}
