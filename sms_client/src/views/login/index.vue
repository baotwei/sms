          ...
          <div>
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
              <el-form-item label="用户名" prop="id">
                <el-input v-model.number="ruleForm.id" placeholder="请输入用户名"  prefix-icon="el-icon-lollipop"></el-input>
              </el-form-item>
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.get('http://localhost:8084/info/getCurrentTerm').then(function (resp) {
            sessionStorage.setItem("currentTerm", resp.data)
          })

          axios.get('http://localhost:8084/info/getForbidCourseSelection').then(function (resp) {
          ...
          if (that.ruleForm.type === 'admin' || that.ruleForm.type === 'teacher') {
            ...
            console.log(form)
            axios.post("http://localhost:8084/teacher/login", form).then(function (resp) {
              console.log("教师登陆验证信息：" + resp.data)
              check = resp.data
              if (check === true) {
                axios.get("http://localhost:8084/teacher/findByTno/" + that.ruleForm.id).then(function (resp) {
                  ...
                  sessionStorage.setItem("tno", resp.data.tno)


                  console.log('name: ' + name + ' ' + that.ruleForm.type + ' ' + resp.data.tno)
            ...
            axios.post("http://localhost:8084/student/login", form).then(function (resp) {
              console.log("学生登录验证信息：" + resp.data)
              check = resp.data
              if (check === true) {
                axios.get("http://localhost:8084/student/findById/" + that.ruleForm.id).then(function (resp) {
                  ...
                  sessionStorage.setItem("sno", resp.data.sno)
